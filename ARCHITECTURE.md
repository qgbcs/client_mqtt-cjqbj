...
`client_service.rpc()` writes a bounded in-memory diagnostic log. The top-bar RPC logs page polls, displays selectable/copyable topic, reply topic, key presence/format/normalized length, request phase, elapsed time, response envelope, and per-broker timeout states. Wi-Fi output is selectable/copyable. Key contents, Aliyun values, and submitted RPC source are excluded.

Online probing is controlled by shared config (`online_probe_enabled`, `online_probe_interval`). Health is tracked per target. A successful feature RPC refreshes the target's last-success/last-probe timestamp, so periodic `online()` probes wait until the configured interval elapses and never overlap an in-flight RPC. When probing is disabled, the UI retains and labels the most recent known online/offline result.
